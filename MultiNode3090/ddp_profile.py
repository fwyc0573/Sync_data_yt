...
local_rank =  int(os.environ['LOCAL_RANK']) # FLAGS.local_rank
...
if local_rank == 0:
    result = round((ee - ss)/FLAGS.repeat*1000, 2)
    # Construct the filename from the provided arguments
    filename = f"{FLAGS.model}_b{FLAGS.batchsize}_steptime{result}ms.txt"

    # Write the result to the file
    with open(filename, 'w') as file:
        file.write(str(result))

    print(f"File {filename} has been created with the result: {result}")
